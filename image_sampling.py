...
def main(argv):
    ...
    z = z0.detach().clone()
    z0_ = flow.get_z0(torch.zeros(sampling_shape_, device=device), train=False).to(
        device
    )
    z_ = z0_.detach().clone()
    class_labels = class_labels_ = None
    if config.data.num_classes:
        class_labels = torch.eye(config.data.num_classes, device=device)[
            torch.randint(0, config.data.num_classes, (config.eval.batch_size,))
        ]
        class_idx = config.sampling.class_label
        if class_idx is not None and class_idx >= 0:
            class_labels[:, :] = 0
            class_labels[:, class_idx] = 1
        class_labels_ = torch.eye(config.data.num_classes, device=device)[
            torch.randint(0, config.data.num_classes, (1,))
        ]
    ...
    if workdir.endswith(".pth"):
        ...
        ckpt_list = sorted(
            ckpt_list, key=lambda x: int(re.findall(r"\d+", x)[0])
        )  # sort by number
        ...
        print(
            "sample from: ", ckpt_list, "begin_ckpt:", begin_ckpt, "end_ckpt:", end_ckpt
        )
    ...
    for ckpt in ckpt_list[begin_ckpt - 1 : end_ckpt]:
        ...
        save_image(
            x,
            os.path.join(eval_dir, f"{flow.use_ode_sampler}_sample_{nfe}.png"),
            nrow=8,
        )
        ...
        for N in [100]:
            ...
            dt = 1.0 / N
            sampling_fn = sampling.get_flow_sampler(
                flow, sampling_shape, inverse_scaler, clip_denoised=False, device=device
            )
            with torch.no_grad():
                x, nfe, (x_h, t_h) = sampling_fn(
                    score_model, z=z, return_xh=True, progress=True, label=class_labels
                )
            x = scaler(x)
            v_final = (x - z).cpu()  # [-1, 1]
            straightness = []
        ...
        for N in [100]:
            flow.sample_N = N
            sampling_fn = sampling.get_flow_sampler(
                flow, sampling_shape_, inverse_scaler, device=device
            )

            with torch.no_grad():
                x, nfe, (x_h, t_h) = sampling_fn(
                    score_model, z=z_, return_xh=True, label=class_labels_
                )
            ...
            for position in range(pixels_h[0].shape[0]):
                plt.plot(
                    t_h,
                    [tensor[position] for tensor in pixels_h],
                    label=f"pixel {position}",
                )
            ...
            plt.grid(True)
            plt.savefig(
                os.path.join(eval_dir, f"{flow.use_ode_sampler}_pixel_traj_{nfe}.png"),
                dpi=600,
                bbox_inches="tight",
            )  # dpi determines resolution, bbox_inches ensures the entire plot is saved
            plt.show()
